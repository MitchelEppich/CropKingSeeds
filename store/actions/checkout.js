...
let shippingMethods = [
    ...
    },
    {
        type: "Regular Shipping (No Tracking)",
        tag: "Regular Shipping",
        description: "Approx. 7 to 14 business days within North America and up to 21 days overseas.",
        price: 10
    },
    {
        ...
        tag: "Express Registered with Tracking",
        description: "Approx. 7 to 14 business days within North America and up to 21 days overseas.",
        note: "This option guarantees Delivery with a Reshipment if your order is redirected.",
        price: 30
    },
    {
        type: "Regular Shipping (No Tracking)",
    ...
    const objects = {
        ...
        modifyOrderDetails: input => {
            ...
            if (
                ...
                if (_requestUpdateOfGroup.group == "payment" && _group == "shipping") {
                    if (_orderDetails["billing"] != null && !_orderDetails["billing"].readOnly)
                        _orderDetails["billing"] = {
                            ..._orderDetails["shipping"],
                            readOnly: true
                        };
                    _orderDetails[_requestUpdateOfGroup.group]["updateRequested"] = true;
                } else _orderDetails[_requestUpdateOfGroup.group]["updateRequested"] = true;
            }

            if (_group != null) {
                if (_orderDetails[_group] == null) _orderDetails[_group] = {};
                _orderDetails[_group][_key] = _tag == null ? _value : { value: _value, tag: _tag };
            ...
            } else if (_tag != null) _orderDetails[_key] = { value: _value, tag: _tag };
            ...
            return async dispatch => {
                ...
                switch (_action) {
                    case "APPEND":
                        ...
                        _coupon = (await makePromise(execute(link, operation))).data.getCoupon;
        ...
        processOrder: input => {
            return async dispatch => {
                ...
                if (ccResponse != null) {
                    if (ccResponse.currency != null) _orderDetails.payment.currency.value = ccResponse.currency;
...
let processPayment = async (details, uri) => {
    return await new Promise(async (resolve, reject) => {
        ...
        let amount = (_payment.orderTotal.value * (_payment.currency.value == "USD" ? _currency.convert : 1)).toFixed(
            2
        );

        let currency = _payment.currency.value == "USD" ? "CAD" : _payment.currency.value;
...
let buildOrderPost = orderDetails => {
    ...
    for (let key of Object.keys(_orderDetails)) {
        ...
        for (let _key of Object.keys(_orderDetails[key])) {
            if (_key == "undefined" || _key == "ccExpireMonth" || _key == "ccExpireYear") continue;
            ...
            if (obj.tag != null && obj.tag.includes(" ")) {
                ...
                if (prefix == "Bill" && _key == "Postal_Zip_Code") _key = "PostalZipCode";
                if (prefix == "Bill" && _key == "PhoneNum") _key = "Phone";
                let $key = (["Shipped_Type", "Shipping"].includes(_key) ? "" : prefix) + _key;
                let $value = obj.value;
                if (_key == "prov_tax" || _key == "tax")
                    $value *= _orderDetails.payment.cartTotal.value + _orderDetails.payment.shippingFee.value;
