...
class Index extends Component {
    componentDidMount() {
        ...
        document.getElementById("copyLinkButton").addEventListener("click", () => {
            let link = document.querySelector("#linkText");
            link.select();
        ...
        for (let i = 0; i < 5; i++) {
            arr.push(
                <FontAwesomeIcon
                    key={i}
                    icon={faCannabis}
                    className={`text-red-dark fa-lg mx-1 ${Math.round(rating) <= i ? "opacity-25" : ""}`}
                />
            );
        }
                ...
                <div className="p-8 lg:px-4 lg:py-8 md:px-4 md:py-8 sm:px-2 sm:py-8">
                    ...
                    <input
                        ...
                        value={
                            ...
                            this.props.viewProduct.currentProduct.name.toLowerCase().replace(/ /g, "-")
                        }
                    />
                </div>
            </Layout>
