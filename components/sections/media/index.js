...
import { faAngleDown, faCaretDown, faCaretUp, faMapMarkerAlt } from "@fortawesome/free-solid-svg-icons";
...
const Media = props => {
    ...
    let showNewsEntry = items => {
        ...
        for (let item of items) {
            let { body, category, date, location, locationUrl, imageUrl, sponsored, title, url, _id } = item;
            ...
            arr.push(
                <div
                    ...
                    className={`w-main mx-auto mt-4 rounded shadow-md relative ${active ? "pb-2" : ""}`}>
                    <div
                        onClick={() => {
                    ...
                    {active ? (
                        <div style={{ background: "rgb(250, 250, 250)" }} className="p-2 inline-flex pt-2 relative">
                            <div className="px-2 ml-4">
                                <img src={imageUrl} className="w-100 h-100 overflower-hidden" style={styleImg} />
                            </div>
