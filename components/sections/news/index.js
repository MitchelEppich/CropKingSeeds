...
import { faCannabis, faMapMarkerAlt, faDollarSign } from "@fortawesome/free-solid-svg-icons";
...
const news = props => {
    ...
    let showEvents = props.misc.featuredNews.map((item, index) => {
        let { body, category, date, location, locationUrl, sponsored, title, url } = item;
            ...
            <div
                onMouseEnter={() => {
                    props.setCurrentEvent({
                        index: index,
                        currentEventObj: props.misc.currentEventObj,
                ...
                <div className="w-full py-2 inline-flex">
                    ...
                    <div className="w-1/4 items-center flex justify-end">
                        ...
                        <p className="justify-end flex text-right opacity-75 font-bold text-sm">{category}</p>
                    </div>
                </div>
                <div className="inline-flex w-full items-center flex">
                    ...
                    {location != null ? (
                        <a
                            ...
                            <FontAwesomeIcon icon={faMapMarkerAlt} className="opacity-50" /> {location}
                        </a>
                    ) : null}
                </div>
