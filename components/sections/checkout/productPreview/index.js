...
const ProductPreview = props => {
                    ...
                    <div className="inline-flex w-full">
                        ...
                        <div className="mt-1 p-2 pt-2 w-2/3 relative">
                            ...
                            <div className="sm:mt-0 mt-2 w-full inline-flex items-center flex">
                                ...
                                <div className="px-2 sm:text-left sm:w-full md:w-full md:text-left font-bold w-1/2 text-grey uppercase text-sm text-right">
                                    {currency != null
                                        ? `${currency.symbol}${(currency.convert * (_item.per || 0)).toFixed(2)}`
                                        : ""}
                                </div>
                            </div>
                            {hasSale ? (
                                <div className="sm:mt-0 mt-2 w-full inline-flex items-center flex">
                                    ...
                                    <div className="px-2 sm:text-left sm:w-full md:w-full md:text-left font-bold w-1/2 text-red-dark uppercase text-sm text-right">
                                        {currency != null
                                            ? `${currency.symbol}${(currency.convert * (_item.sale || 0)).toFixed(2)}`
                                            : ""}
                                    </div>
                                </div>
                            ) : null}
                            <div className="w-full absolute z-50 sm:flex-col pin-l pin-b mb-4 p-2 sm:p-1 sm:mb-0 flex items-center inline-flex ">
                                ...
                                <div className="text-right w-full mt-1 sm:mt-2 md:mt-2 items-center">
                                    <p className={`font-bold ${hasSale ? "text-red-dark" : "text-grey"} text-lg`}>
                                        {currency != null
                                            ? `${currency.symbol}${(currency.convert * (_item.price || 0)).toFixed(2)}`
                                            : ""}
                                    </p>
                                </div>
                            </div>
    ...
    return (
        <div className="w-full mt-6 mb-2 px-8 sm:px-4">
            <h2 className="text-3/5xl font-extrabold opacity-50 mt-8 mb-4 text-center text-black">Shopping Cart</h2>
            ...
            <div className="w-full inline-flex flex-wrap mb-4 mt-2">
                {_itemscart.length > 0 ? (
                    ...
                    <div className="w-full text-center text-base">
                        <p className="text-3xl mb-2 mt-12 font-extrabold">Sorry, your cart is empty!</p>
