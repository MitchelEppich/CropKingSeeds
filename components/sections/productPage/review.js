...
const review = props => {
  ...
  return (
    <div className="lg:block md:block sm:block sm:w-full w-full sm:ml-0 sm:pl-2 pl-2 mb-2 pb-4 shadow rounded p-2 inline-flex">
      <div className="pr-4 w-1/4 md:w-full sm:w-full lg:w-full md:inline-flex sm:inline-flex lg:inline-flex md:pr-0 sm:flex-col sm:pr-0 lg:pr-0">
        <div className="w-full pl-2">
          <p className="font-bold text-lg mb-1 capitalize">{props.name}</p>
          <p className="text-xs w-full">{moment(props.date).fromNow()}</p>
        </div>
        <div className="sm:text-right sm:justify-end sm:mt-2 sm:w-full md:text-right md:justify-end md:flex lg:text-right lg:justify-end lg:flex sm:w-1/2 md:w-1/2 lg:w-1/2">
