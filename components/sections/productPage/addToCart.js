        ...
        <div className="w-full">
            <div className="w-container sm:w-full md:w-full mx-auto mt-4">
                <div className="inline-flex items-center flex w-full my-2 opacity-50">
                    <div className="">
                        <p className="w-100 text-left text-sm text-grey font-bold mx-auto">Price per Pack: </p>
                    </div>
                    <p className="font-bold text-xl text-grey-darkest ml-4">
                        {currency != null
                            ? `${currency.symbol}${(
                                  currency.convert * currentProduct.price[props.shop.quickAddToCartQty]
                              ).toFixed(2)}`
                            : ""}
                    </p>
                </div>
            ...
            <div className="w-1/4 h-8 sm:w-full md:w-full w-container mx-auto">
                <div className="inline-flex flex my-2 items-center ">
                    <div>
                        <p className="w-100 text-left text-sm text-grey font-bold mx-auto">Total Price: </p>
                    </div>
                    ...
                    <p className="font-bold text-xl text-grey-darkest ml-4">
                        {currency != null
                            ? `${currency.symbol}${(
                                  currency.convert *
                                  (currentProduct.price[props.shop.quickAddToCartQty] * props.cart.potentialQuantity)
                              ).toFixed(2)}`
                            : ""}
                    </p>
                </div>
                ...
                <div className="w-full inline-flex">
                    <div className="w-1/2 mr-2 pt-2">
                        <button
                            ...
                            onClick={() => {
                                let _identifier = currentProduct.sotiId + [5, 10, 25][props.shop.quickAddToCartQty];
                                props.modifyCart({
