...
if ($report_number) {
      ...
      if ($paramSearch != '') {
        ...
        foreach ( $result->fetchAll() as $pqry) {
            ...
            $ptot = '<span class="label label-success">'.(number_format($ptotget->rowCount())).'<span>';
            $tmpcm = array(
                $commission_date,
                $ptot,
                $commission_amount
            ...
            if ( $aff_inv == 1) {
                $tmpcm[] = '<form method="POST" action="invoice.php" target="_blank"><input name="csrf_token" value="'.$_SESSION['csrf_token'].'" type="hidden" /><input type="hidden" value="'.$record_id.'" name="stamp"><input type="submit" value="'.$invoice_button.'" class="btn btn-xs btn-danger"></form>';
            }
