...
const resolvers = {
    Query: {
        ...
        getRandomWinnerBetweenDates: async (_, { input }) => {
            let _start = input.startDate == "today" ? moment().format("YYYY-MM-DD") : input.startDate;
            let _end = input.endDate == "today" ? moment().format("YYYY-MM-DD") : input.endDate;
        ...
        getCoupon: async (_, input) => {
            ...
            if (company != "cropkingseeds.com") return { error: "Invalid Code Entered" };
            ...
            if (startDate != null && endDate != null && !moment().isBetween(startDate, endDate))
            ...
            if (usageLimit != -1) {
                ...
                if (res.length >= usageLimit) return { error: "Coupon Usage Limit Reached of " + usageLimit };
            }
