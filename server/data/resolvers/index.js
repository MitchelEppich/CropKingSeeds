...
const resolvers = {
    Query: {
        ...
        getBitcoinData: async (_, { input }) => {
            return await axios
                .get(`https://blockchain.info/tobtc?currency=${input.currency}&value=${input.value}`)
                .then(res => {
                    return res.data;
                });
        },
        getExchangeRates: async _ => {
            return await axios.get("https://www.cksoti.com/checkexchangerate").then(res => {
                res = JSON.parse(
                    convert.xml2json(
                        res.data
                            .toLowerCase()
    ...
    Mutation: {
        ...
        processPayment: async (_, { input }) => {
            ...
            if (pivotal.limit == -1 || pivotal.available - amount > 0) response = await processPivotal(input);
...
let processBambora = async input => {
    ...
    let beanstream = require("beanstream-node")("225812615", "32f630b674F24A73941Ee23b9237874A");
...
let getProcessorUsage = async () => {
    ...
    let bambora = JSON.parse(
        convert.xml2json(
            (await request({
                ...
                .toLowerCase()
                .replace("<br/><?xml?>", "</Data>")
                .replace('<?xml version="1.0" encoding="utf-8"?>', '<?xml version="1.0" encoding="utf-8"?><Data>'),
            { compact: true, spaces: 4 }
        )
    )["Data"];
    bambora = {
        current: parseFloat(bambora.currentamount._text.replace(/[,$]/g, "")) || -1,
        available: parseFloat(bambora.availableamount._text.replace(/[,$]/g, "")) || -1,
        limit: parseFloat(bambora.amountlimitperday._text.replace(/[,$]/g, "")) || -1
    };
    ...
    pivotal = {
        ...
        available: parseFloat(pivotal.availableamount._text.replace(/[,$]/g, "")) || -1,
        limit: parseFloat(pivotal.amountlimitperday._text.replace(/[,$]/g, "")) || -1
    };
